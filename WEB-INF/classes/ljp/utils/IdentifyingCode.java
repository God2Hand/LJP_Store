...
public class IdentifyingCode extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 在内存中生成图片（纸），没有设置背景颜色，画填充的矩形，并且和纸的大小相同，并且有颜色；
...
//		Graphics g = image.getGraphics();
		Graphics2D g = (Graphics2D)image.getGraphics();	//为了扭曲字体的效果
		//设置灰色
		...
		g.drawRect(0, 0, width-1, height-1);//这里-1作为内边框，不然只显示左上两条线
		...
		for (int i = 0; i < 4; i++) {
			...
			g.rotate(-hudu, x, y);	//由于第一个字符的旋转会影响到第二个字符，所以用这个来取消影响
			
			x += 20;
		}
		
		// session : 存入到session中
		request.getSession().removeAttribute("identifyingCode");
		request.getSession().setAttribute("identifyingCode", sb.toString());
