...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">
			...
			function send(oid) {
				if (confirm("确认发货吗？")) {
					location.href = "${pageContext.request.contextPath }/adminOrder?method=updateState&oid=" + oid;
				}
			}
 			function showDetail(oid, addr, tele){
				//发送ajax
				$.post("${pageContext.request.contextPath}/adminOrder",{"method":"showDetail","oid":oid},function(d){
					var s = ""
					s += "<font size='-3'>收货地址：" + addr + "</font><br />";
					s += "<font size='-3'>联系电话：" + tele + "</font><br />";
					
					s += "<table border='1' width='99%' >";
					s += "<tr><th align='center' >商品名称</th><th align='center' >购买数量</th></tr>"
					$(d).each(function(){
						s += "<tr><td align='center' >" + this.product.pname + "</td><td align='center' >" + this.count + "</td></tr>"
					});
					s += "</table>";
									...
									<c:forEach items="${list }" var="o" varStatus="vs" >
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												...
												<c:if test="${o.state == 1}">
													<a href="javascript:void(0)" onclick="send('${o.oid}')" >
														<font color='blue' ><u>去发货</u></font>
													</a>
												</c:if>
												<c:if test="${o.state == 2}">待收货</c:if>
												<c:if test="${o.state == 3}">已完成</c:if>
												<c:if test="${o.state == 4}">已完成（用户已删除）</c:if>
											
											</td>
											<td align="center" style="HEIGHT: 22px">
												<input type="button" value="订单详情" onclick="showDetail('${o.oid}', '${o.address }', '${o.telephone }')"/>
